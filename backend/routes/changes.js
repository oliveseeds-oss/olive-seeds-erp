...
router.post('/', authenticate, async (req, res) => {
  try {
    const { module, record_id, field_name, current_value, requested_value, reason } = req.body;
    const [r] = await db.query('INSERT INTO change_requests (requested_by, module, record_id, field_name, current_value, requested_value, reason) VALUES (?,?,?,?,?,?,?)',
      [req.user.id, module, record_id, field_name, current_value, requested_value, reason]);
...
router.patch('/:id/approve', authenticate, requireAdmin, async (req, res) => {
  try {
    const [reqs] = await db.query('SELECT * FROM change_requests WHERE id=?', [req.params.id]);
    if (!reqs.length) return res.status(404).json({ error:'Not found' });
    const cr = reqs[0];
    await db.query(`UPDATE ${cr.module} SET ${cr.field_name}=? WHERE id=?`, [cr.requested_value, cr.record_id]);
    await db.query('UPDATE change_requests SET status="approved", reviewed_by=?, reviewed_at=NOW(), review_notes=? WHERE id=?', [req.user.id, req.body.notes, req.params.id]);
...
router.patch('/:id/reject', authenticate, requireAdmin, async (req, res) => {
  try {
    await db.query('UPDATE change_requests SET status="rejected", reviewed_by=?, reviewed_at=NOW(), review_notes=? WHERE id=?', [req.user.id, req.body.notes, req.params.id]);
