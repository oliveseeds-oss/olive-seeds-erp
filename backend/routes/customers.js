...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    let query = 'SELECT * FROM customers WHERE is_active = 1 AND deleted_at IS NULL';
    const params = [];
    if (search) {
    ...
    const [[{ total }]] = await db.query('SELECT COUNT(*) as total FROM customers WHERE is_active = 1 AND deleted_at IS NULL');
...
router.get('/:id', authenticate, async (req, res) => {
  try {
    const [customers] = await db.query('SELECT * FROM customers WHERE (id = ? OR customer_id = ?) AND deleted_at IS NULL', [req.params.id, req.params.id]);
    if (!customers.length) return res.status(404).json({ error: 'Customer not found' });
    const [orders] = await db.query('SELECT order_id, total, status, created_at FROM orders WHERE customer_id = ? AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 20', [customers[0].id]);
  ...
  try {
    ...
    const [result] = await db.query(
      ...
      [
        ...
        currency || 'INR', language || 'English', customer_group || null, credit_limit || 0,
        outstanding_balance || 0, notes || null, alternate_phone || null
      ]
    );
